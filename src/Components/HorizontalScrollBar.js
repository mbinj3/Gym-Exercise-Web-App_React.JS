...
const LeftArrow = () => {
  ...
  return (
    <Typography onClick={() => scrollPrev()} className={`left-arrow ${isFirstItemVisible ? 'disabled' : ''}`}>
...
const RightArrow = () => {
  ...
  return (
    <Typography onClick={() => scrollNext()} className={`right-arrow ${isLastItemVisible ? 'disabled' : ''}`}>
      <img src={RightArrowIcon} alt="right-arrow" />
    </Typography>
  );
};
